...
func TestYAMLParseFile(t *testing.T) {
	...
	files := file.Glob(t, filepath.Join(template.TestingDefaultFixtureDir, "*.yaml"))
	...
	for _, f := range files {
		t.Run(f.Base, func(t *testing.T) {
			...
			p, err := template.NewYAML(template.WithIgnoreMissingValue(true))
...
func TestYAMLApply(t *testing.T) {
	files := file.Glob(t, filepath.Join(template.TestingDefaultFixtureDir, "pass-*.yaml"))
