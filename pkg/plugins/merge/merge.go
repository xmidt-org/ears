...
// SPDX-License-Identifier: Apache-2.0package transform
...
import (
	...
	pkgmerge "github.com/xmidt-org/ears/pkg/filter/merge"
...
func NewFilterer(tid tenant.Id, plugin string, name string, config interface{}, secrets secret.Vault, tableSyncer syncer.DeltaSyncer) (filter.Filterer, error) {
	return pkgmerge.NewFilter(tid, plugin, name, config, secrets, tableSyncer)
}
