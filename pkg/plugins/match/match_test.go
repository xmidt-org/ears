	...
	for _, tc := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			for _, mode := range []match.ModeType{match.ModeAllow, match.ModeDeny} {
				...
				f, err := p.NewFilterer(tenant.Id{AppId: "myapp", OrgId: "myorg"}, "match", "mymatch", match.Config{
					Mode:    mode,
				...
				for _, in := range succeed {
					t.Run(mode.String()+"/"+in, func(t *testing.T) {
						a := NewWithT(t)
						e, err := event.New(ctx, in, event.FailOnNack(t))
						a.Expect(err).To(BeNil())
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, cc := range configCases {
				t.Run(cc.name, func(t *testing.T) {
					...
					f, err := p.NewFilterer(tenant.Id{AppId: "myapp", OrgId: "myorg"}, "match", "mymatch", cc.config, nil, nil)
					if cc.name == "invalid" {
...
func TestNilFilter(t *testing.T) {
	...
	e, err := event.New(context.Background(), nil, event.FailOnAck(t))
	a.Expect(err).To(BeNil())

	evts := f.Filter(e)
	a.Expect(evts).To(BeNil())
